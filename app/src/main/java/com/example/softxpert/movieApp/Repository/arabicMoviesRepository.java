...
public class arabicMoviesRepository {
//this class for allMoviesRepository

    private static arabicMoviesRepository instance;
        private final MutableLiveData<List<movieModel>> movies;

    public arabicMoviesRepository() {

        movies=new MutableLiveData<>();

    }

    public static arabicMoviesRepository getInstance() {
        if (instance == null){
        instance =new arabicMoviesRepository();
    }
        return instance;
    }
    ...
    //method to return all movies using restAPI
        public void moviesAPI(int page){
            ...
            AppExecutors.getInstance().NetworkIO().submit(() -> CallMovieAPI(page));



        }

    private void CallMovieAPI(int page) {
        Call<allMoviesResponse>  allMovies=Servicey.getMovieApi().ArabicMovies(Credentials.API_KEY,"EG","popularity.desc",page);
