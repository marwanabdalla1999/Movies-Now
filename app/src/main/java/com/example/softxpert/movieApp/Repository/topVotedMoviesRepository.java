...
public class topVotedMoviesRepository {
//this class for allMoviesRepository

    private static topVotedMoviesRepository instance;
        private final MutableLiveData<List<movieModel>> movies;

    public topVotedMoviesRepository() {

        movies=new MutableLiveData<>();

    }

    public static topVotedMoviesRepository getInstance() {
        if (instance == null){
        instance =new topVotedMoviesRepository();
    }
        return instance;
    }
    ...
    private void CallMovieAPI(int page,String sortBy) {
        Call<allMoviesResponse>  allMovies=Servicey.getMovieApi().topRated(Credentials.API_KEY,page,"en-US");
